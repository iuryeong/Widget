...
function renderFeedItem(item) {
  switch (item.type) {
      ...
      if (item.isPreMarket) {
        ...
        chartArea = `
          <div style="margin-top: 8px; text-align: center;">
            <img src="${
              item.chartUrl
            }?t=${new Date().getTime()}" alt="차트" style="width: 100%; height: auto; border-radius: 4px;" />
          </div>`;
      }
    ...
    case "todo":
      // 할 일 목록 HTML 생성
      const todoListHtml = item.items
        .map(
          (todo) => `
        <div class="todo-item" style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
          <input type="checkbox" class="todo-check" data-id="${todo.id}" ${
            todo.done ? "checked" : ""
          } style="cursor: pointer;">
          
          <input type="text" class="todo-text" data-id="${todo.id}" value="${
            todo.text
          }" 
            style="flex: 1; border: none; border-bottom: 1px solid #eee; padding: 4px; outline: none; font-size: 13px; color: ${
              todo.done ? "#aaa" : "#333"
            }; text-decoration: ${
          ...
          <button class="todo-delete" data-id="${
            todo.id
          }" style="border: none; background: none; cursor: pointer; color: #ff6b6b; font-size: 14px;">✕</button>
        </div>
      `
        )
        .join("");
  ...
  if (feedContainer) {
    ...
    feedContainer.addEventListener("input", (e) => {
      ...
      if (e.target.classList.contains("todo-text")) {
        ...
        todoTimeoutId = setTimeout(async () => {
          const { userTodos = [] } = await chrome.storage.sync.get([
            "userTodos",
          ]);
    ...
    feedContainer.addEventListener("click", async (e) => {
      ...
      if (e.target.classList.contains("todo-check")) {
        ...
        if (textInput) {
          textInput.style.textDecoration = e.target.checked
            ? "line-through"
            : "none";
